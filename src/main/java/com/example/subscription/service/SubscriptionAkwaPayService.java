 ...
 * lives at /api/webhooks/akwapay/subscription, so it must use the whsec_
 * AkwaPay shows for THAT specific URL in the dashboard —
 * app.akwapay.webhook-secret.subscription. ScanPurchaseAkwaPayService's
 * /api/webhooks/akwapay/scan endpoint has its own, different secret
 * (app.akwapay.webhook-secret.scan). Do not point both properties at the
 * same value, and do not collapse them back into one shared
 * app.akwapay.webhook-secret — verifySignature() will silently reject every
 * event on whichever endpoint has the wrong secret.
 *
 * ─────────────────────────────────────────────────────────────────────────
...
public class SubscriptionAkwaPayService {
    ...
    @Value("${app.akwapay.webhook-secret.subscription}")   private String webhookSecret;
